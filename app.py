...
# ---------------------------------------------------------------------------
# Daily Brief — 내 보유 종목 오늘 포커스 (Zone 1 ↔ Zone 2 사이)
# ---------------------------------------------------------------------------

def _render_brief_holdings_focus_section():
    """Daily Brief 의 '내 보유 종목 — 오늘 포커스' 섹션 — portfolio-first 브리프 핵심.

    portfolio.json 의 의미 비중 보유 종목을 비중 내림차순으로 콤팩트 카드(~5줄)로
    보여준다. 카드는 종목명·비중·레버리지·수익률을 헤더로, 익스포저 테마 한 줄,
    그리고 holdings_briefing 의 today_focus_ko·today_action_ko·upcoming_catalysts_ko 를
    노출한다 (LLM·rule-based 어느 쪽이든 항상 채워짐).

    이 섹션은 BRIEF — 풍부한 summary/key_drivers/risks/portfolio_note 는 Portfolio
    리뷰 페이지에 그대로 남겨두고, 여기서는 '오늘' 본질만 보여준다.
    어떤 실패도 위로 던지지 않으며, 데이터 부재 시 calm 한 안내 카드로 대체.
    """
    import json as _json

    st.markdown(
        '<div class="section-title">내 보유 종목 — 오늘 포커스</div>',
        unsafe_allow_html=True,
    )
    ...
    try:
        ...
        log.debug(f"brief holdings focus — portfolio 로드 실패: {e}")
        st.markdown(
            '<div class="card"><div class="pick-type">'
            '보유 종목 데이터(portfolio.json)를 읽을 수 없어 오늘 포커스를 표시할 수 없습니다.'
            '</div></div>',
            unsafe_allow_html=True,
        )
        return
    ...
    if not pf.get("available"):
        st.markdown(
            '<div class="card"><div class="pick-type">'
            '보유 종목 데이터가 없습니다 — data/portfolio.json 을 확인하십시오.'
            '</div></div>',
            unsafe_allow_html=True,
        )
        return
    ...
    if not meaningful:
        st.markdown(
            '<div class="card"><div class="pick-type">'
            '의미 비중(1% 이상) 보유 종목이 없습니다.'
            '</div></div>',
            unsafe_allow_html=True,
        )
        return

    # 2) 최신 holdings_briefing 행 조회
    briefings_by_ticker: dict[str, Any] = {}
    try:
        with db.db_session() as conn:
            briefing_rows = db.fetch_holdings_briefings(conn)
        for r in (briefing_rows or []):
            try:
                briefings_by_ticker[r["ticker"]] = r
            except Exception:
                pass
    except Exception as e:
        log.debug(f"brief holdings focus — briefing 조회 실패: {e}")
    ...
    for h in meaningful:
        ...
        nw_str = f"{nw:.1f}%" if nw is not None else "—"
        if ret is None:
            ret_html = '<span style="color:var(--muted);">—</span>'
        else:
            ret_color = ("#22C55E" if ret > 0
                         else "#EF4444" if ret < 0 else "var(--muted)")
            ret_html = f'<b style="color:{ret_color};">{ret:+.1f}%</b>'
        lev_badge = (
            '<span class="tag" style="background:#7F1D1D; color:#FCA5A5; '
            'border:1px solid #991B1B; margin-left:6px;">레버리지</span>'
            if is_lev else ""
        )
        ...
        header = (
            '<div style="display:flex; align-items:center; '
            'justify-content:space-between; gap:10px; flex-wrap:wrap;">'
            f'<div><b style="font-size:14.5px;">{name}</b> '
            f'<span style="color:var(--muted); font-size:12px;">{ticker}</span>'
            f'{lev_badge}</div>'
            f'<div style="font-size:13px;">비중 <b>{nw_str}</b> · 수익률 {ret_html}</div>'
            '</div>'
        )
        ...
        theme = _g(row, "exposure_theme") or ""
        focus = _g(row, "today_focus_ko") or ""
        action = _g(row, "today_action_ko") or ""

        raw_uc = _g(row, "upcoming_catalysts_ko")
        catalysts: list[str] = []
        if isinstance(raw_uc, list):
            catalysts = raw_uc
        elif raw_uc:
            try:
                parsed = _json.loads(raw_uc)
                catalysts = parsed if isinstance(parsed, list) else []
            except Exception:
                catalysts = []
        # 너무 길어지지 않게 최대 2개만 인라인 노출
        catalysts_short = [c for c in (catalysts or []) if c][:2]
        ...
        if focus:
            focus_html = (
                ...
                '<span class="tag" style="background:#1E293B; color:#93C5FD; '
                'border:1px solid #1E40AF; flex-shrink:0;">오늘 포커스</span>'
                '<span style="font-size:13px; color:var(--text-mid); '
                f'line-height:1.55;">{focus}</span>'
                '</div>'
            )
        ...
        if action:
            action_html = (
                ...
                '<span class="tag" style="background:#7C2D12; color:#FCD34D; '
                'border:1px solid #92400E; flex-shrink:0;">오늘 액션</span>'
                '<span style="font-size:13px; color:var(--text); '
                f'line-height:1.55;">{action}</span>'
                '</div>'
            )

        catalysts_html = ""
        if catalysts_short:
            txt = ", ".join(catalysts_short)
            catalysts_html = (
                '<div style="margin-top:6px; font-size:12px; '
                'color:var(--muted); line-height:1.5;">'
                '<span style="text-transform:uppercase; letter-spacing:0.04em; '
                'margin-right:6px;">차기 변수</span>'
                f'{txt}'
                '</div>'
            )
    ...
    # Honesty footer — 구조·사이클 기반임을 명시 (실시간 KR 뉴스 아님)
    st.markdown(
        '<div style="font-size:11.5px; color:var(--muted); margin-top:4px; '
        'opacity:0.85;">'
        '구조적·사이클 기반 포커스 — 실시간 KR 뉴스 통합은 별도 단계입니다.'
        '</div>',
        unsafe_allow_html=True,
    )
...
def render_today_brief():
    ...
    # ════ Zone 1.5 — 내 보유 종목 오늘 포커스 ════════════════════════
    # portfolio-first 브리프의 핵심. 의미 비중 보유 종목별로 오늘 포커스와
    # 오늘 액션(비중·레버리지·사이클을 엮은 한 줄)을 콤팩트 카드로 노출.
    _render_brief_holdings_focus_section()
